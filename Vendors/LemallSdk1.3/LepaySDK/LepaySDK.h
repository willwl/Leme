...
#define LepayLocaleGet @[@"unknow",@"America",@"hk"]
 ...
 *  @param tradeInfoDic    支付信息（详情见文档）
 *  @param schemeStr       1、订单国内支付传 微信AppId wx****** 2、国外支付传 kLepaySDKForeignPay
...
/*
 * @brief   iap功能必须先注册平台类型（功能与老接口一样）
 * @param   平台类型
 */
+ (void)registerPlatformV2:(LepayPlatformType)platform;

/*
 * @brief  根据v2套餐接口文档，重构全量接口（根据会员id、地区码和代理获取list)
 *          wiki地址：http://wiki.letv.cn/pages/viewpage.action?pageId=53888268
 * @param   memberId 会员id，在boss配置完套餐后，对应分配的会员id号
